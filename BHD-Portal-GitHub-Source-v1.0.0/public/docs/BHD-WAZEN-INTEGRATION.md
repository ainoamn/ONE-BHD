...
- [BHD-UNIFIED-LOGIN-AND-APPS.md](https://github.com/ainoamn/ONE-BHD/blob/main/docs/BHD-UNIFIED-LOGIN-AND-APPS.md) — الدليل المرجعي للتنقل الصامت وصيانة الفرق
...
9. **حساب واحد في المتصفح.** وازن لا يفتح `/admin` أو أي صفحة بحساب مختلف عن جلسة `id.bhd-om.com`. عند `callback` امسح كوكي وازن السابقة واربط `bhd_sub` الجديد. أزل دخول جوجل/كلمة المرور المحلي بعد الربط. لتغيير الحساب: خروج وازن + `end-session` ثم دخول من الهوية.
